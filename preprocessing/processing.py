...
def buffer_to_jet(entry, tag = 0, side = 'r', max_entry = None, pix = 25):
   ...
   # if (entry['JetAK8_Eta'] < -10) | (entry['JetAK8_Phi'] < -10):
    #    e, p = (entry['PCEta'], entry['PCPhi'])
   # else:
    #e, p = (entry['SubLeadingEta'], entry['SubLeadingPhi'])
    e, p = (entry['jet_eta'], entry['jet_phi'])
    ...
    image = flip_jet(rotate_jet(np.array(entry['images']), -angle, normalizer=4000.0, dim=pix), side)
    e_norm = np.linalg.norm(image)
    return ((image / e_norm).astype('float32'))
