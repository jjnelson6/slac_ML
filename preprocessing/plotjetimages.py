...
def plot_mean_jet(rec, field = 'image', title = 'Average Jet Image',signal=1):
    ...
    im = ax.imshow(np.mean(rec[field][rec['signal']==signal], axis = 0),aspect='auto' ,norm=LogNorm(vmin=.001, vmax=10), extent=(min(rec['jet_eta']),max(rec['jet_eta']),min(rec['jet_phi']),max(rec['jet_phi'])),interpolation='nearest')
    plt.colorbar(im)
