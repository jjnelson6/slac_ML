...
#        ModelCheckpoint('./trainings/final-slac-maxout-hypercube-unnormalized-logloss-cvFold{}.h5'.format(foldN), monitor='val_loss', verbose=True, save_best_only=True)
    	...
    	h = dl.fit(X[train_ix], y[train_ix], batch_size=32, nb_epoch=50, show_accuracy=True, 
    	               ...
    	               [
    	                   ...
    	                   ModelCheckpoint('final-slac-maxout-hypercube-unnormalized-logloss-cvFold{}.h5'.format(foldN), monitor='val_loss', verbose=True, save_best_only=True),

    	                   ROCModelCheckpoint('final-slac-maxout-hypercube-unnormalized-roc-cvFold{}.h5'.format(foldN), X[test_ix], y[test_ix], cube_weights[test_ix], verbose=True)
    	               ],
...
with open('final-slac-maxout-unnormalizedphypercube.yaml', 'wb') as f:
	f.write(dl.to_yaml())
