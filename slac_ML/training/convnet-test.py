...
#        help='are the weights expecting a normed jet image?', required=True)
    ...
    dl = Sequential()
    dl.add(Convolution2D(32, *FILTER_SIZES[0], input_shape=(1, 25, 25), border_mode='valid', W_regularizer=regularizers.l2(0.01)))
    dl.add(Activation('relu'))
    dl.add(MaxPooling2D((2, 2)))

    dl.add(Convolution2D(32, *FILTER_SIZES[1], border_mode='valid', W_regularizer=regularizers.l2(0.01)))
    dl.add(Activation('relu'))
    dl.add(MaxPooling2D((3, 3)))

    dl.add(Convolution2D(32, *FILTER_SIZES[2], border_mode='valid', W_regularizer=regularizers.l2(0.01)))
    dl.add(Activation('relu'))
    dl.add(MaxPooling2D((3, 3)))
   ...
   # dl.load_weights(WEIGHT_FILE)
    ...
    data = np.load(TEST_DATA)['image'][:10000]
